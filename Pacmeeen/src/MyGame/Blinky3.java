...
public class Blinky3 extends GameObject{
	
	private Random r = new Random();
	public static int releaseTimer=400;
	Handler handler;
	public float a,b;
	public int c;
	public int coolDownTimer = 0;
	
	public Blinky3(int x, int y, ID id, Handler handler) {
		super(x,y,id);
						...
						if(Math.abs(a) > Math.abs(b)) {
							...
							if(a>0) {
								...
								if(b>0) {
									...
									int number = r.nextInt(12)+1;		//random number decides which direction Blinky will head in
									...
									else if(number<=12) {		//Blinky is likely to travel up
										velY=-1;
									}
								}
							}
								...
								if(b>0) {
									...
									else if(number<=7) {		//Blinky is most likely to travel left
										velX=-1;
									}
									else if(number<=11) {		//Blinky is likely to travel down
										velY=1;
							...
							if(a>0) {
								...
								if(b>0) {
									...
									else if(number<=11) {		//Blinky is most likely to travel down
										velY=1;
									}
									else if(number==12) {
										velY=-1;
									...
									else if(number<=12) {		//Blinky is most likely to travel up
										velY=-1;
									}
								}
							}
	...
	public boolean turnOpen() {
		//projects path of direction to see if Blinky would collide with map lines if she were to turn
		if(
	...
	public void render(Graphics g) {
		...
		if(HUD.powerUp == true) {
			...
			g.fillOval((int)x+1,(int)y+2+2,4,3);
			g.fillOval((int)x+8+3,(int)y+2+2,4,3);
			//mouth
			g.fillOval((int)x+3,(int)y+9,10,6);
		}
	}
}
