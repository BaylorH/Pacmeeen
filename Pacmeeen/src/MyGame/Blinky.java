...
public class Blinky extends GameObject{
	...
	public static int releaseTimer=200;
	Handler handler;
	float a,b;
	int c;
	
	public Blinky(int x, int y, ID id, Handler handler) {
		super(x,y,id);
						...
						if(Math.abs(a) > Math.abs(b)) {
							...
							if(a>0) {
								...
								if(b>0) {
									...
									int number = r.nextInt(10)+1;		//random number decides which direction Blinky will head in
									...
									else if(number<=9) {		//Blinky is likely to travel down
										velY=1;
									}
									else if(number==10) {
										velY=-1;
									...
									else if(number<=10) {		//Blinky is likely to travel up
										velY=-1;
									}
								}
							}
								...
								if(b>0) {
									...
									else if(number<=10) {		//Blinky is likely to turn up
										velY=-1;
									}
								}
							}
							...
							if(a>0) {
								...
								if(b>0) {
									...
									if(number<=3){		//Blinky is likely to travel right
										velX=1;
									}
									else if(number==4) {
										velX=-1;
									}
									else if(number<=9) {		//Blinky is most likely to travel down
										velY=1;
									}
									else if(number==10) {
										velY=-1;
									...
									else if(number<=10) {		//Blinky is most likely to travel up
										velY=-1;
									}
								}
							}
								...
								if(b>0) {
									...
									else if(number<=4) {		//Blinky is likely to travel left
										velX=-1;
									}
									else if(number<=9) {		//Blinky is most likely to travel down
										velY=1;
	 ...
	 * 		int length of path open below = below
	 * 		int length of path open right = right
	 * 		if(below + right > 100)
	 * 			if(below > right)
	 * 				travel down
	 * 			else
	...
	public void render(Graphics g) {
		...
		if(HUD.powerUp == true) {
			...
			leftEyeBrow.addPoint((int)x+8, (int)y+6); 	//bottom right
			leftEyeBrow.addPoint((int)x+7, (int)y+2); 	//top right
			leftEyeBrow.addPoint((int)x+1, (int)y); 	//top left
			leftEyeBrow.addPoint((int)x+1, (int)y+3); 	//bottom left
			g.fillPolygon(leftEyeBrow);
			
			Polygon rightEyeBrow = new Polygon();
			rightEyeBrow.addPoint((int)x+15, (int)y+3); //bottom right
			rightEyeBrow.addPoint((int)x+15, (int)y); 	//top right
			rightEyeBrow.addPoint((int)x+8, (int)y+2); 	//top left
			rightEyeBrow.addPoint((int)x+8, (int)y+5); 	//bottom left
			g.fillPolygon(rightEyeBrow);
		}
	}
}
